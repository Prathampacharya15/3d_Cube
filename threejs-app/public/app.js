// Declare global variables for scene, camera, renderer, cube object
...
// This function sets up the entire 3D scene
function init() {
  ...
  const geometry = new THREE.BoxGeometry(); // Basic box shape
  const material = new THREE.MeshLambertMaterial({ color: 0xffff00 }); // Yellow color with lighting
  cube = new THREE.Mesh(geometry, material); // Combine shape and material into a mesh
...
// Save current cube state (position and speed) to the database
function saveCube() {
